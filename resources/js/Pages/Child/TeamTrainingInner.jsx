...
export default function TeamTrainingInner({ child, teamItem, subItems = [] }) {
    const [popup, setPopup] = useState(null);

    function handleSubItemClick(item) {
        if (item.content_type === 'quiz') {
            router.get(route('children.team-training.sub-quiz', { child: child.id, teamItem: teamItem.id, subItem: item.id }));
            return;
        }
    ...
    return (
        <AuthenticatedLayout>
            <Head title={`${teamItem.title} — ${child.name}`} />

            <div className="min-h-[calc(100vh-64px)] bg-gray-100">

                {/* Header */}
                {teamItem.header_image ? (
                    <div className="relative shrink-0">
                        <img src={`/storage/${teamItem.header_image}`} alt={teamItem.title} className="w-full object-cover max-h-36" />
                        <div className="absolute top-3 left-4">
                            <Link
                                href={route('children.team-training', child.id)}
                                className="inline-flex items-center gap-2 rounded-lg bg-black/30 hover:bg-black/50 backdrop-blur-sm px-3 py-2 text-sm font-medium text-white transition-colors"
                            >
                    ...
                    <div className="bg-white border-b border-gray-200 px-6 py-4">
                        <div className="relative flex items-center justify-center">
                            ...
                            <div className="flex flex-col items-center gap-1">
                                <img src={`/storage/${teamItem.icon_path}`} alt={teamItem.title} className="h-16 object-contain" />
